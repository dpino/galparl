function BubbleChart(selector, loader) {
    ...
    updateActive = function(id) {
        ...
        if (id.length > 0) {
            chart.loader.renderWordEntries(id);
            return d3.select("#status").html("<h3>Atopáronse <b>" + chart.count + "</b> entradas para a palabra <span class=\"active\">" + id + "</h3>");
        } else {
            // return d3.select("#status").html("<h3>No word is active</h3>");
            return d3.select("#status").html("<h3></h3>");
        }
    ...
    chart.addWord = function(word) {
        this.newWords.push = word;
        this.loadData();
        /*
        var newWord = data[data.length - 1];
        newWord.name = word;
    ...
    chart.init = function() {
      ...
      key = key.replace("/", "");
      this.text = texts.filter(function(t) {
        return t.key === key;
      })[0];
      if (!this.text) {
        this.text = texts[0];
    ...
    chart.loadData = function() {
        var chart = this;
        d3.csv("/data/" + this.text.file, function(data) {
            /*
          for (var i = 0; i < data.length; i++) {
            console.log(data[i]);
          }
          if (chart.newWords.length > 0) {
              for (var i = 0; i < chart.newWords.length; i++) {
                  data.push(chart.newWords[i]);
              }
          }
          */
          // data.push(this.newWords);
          /*
          console.log(data[0]);
          var newWord = new data[0];
          newWord.name = "hello";
          data = [data[0], newWord];
          */
          return chart.display(data);
        });
    }
    chart.display = function(data) {
        if (data) {
            this.data = data;
        }
        return this.plotData(this.selector, this.data, this);
    }
    chart.plotData = function(selector, data, plot) {
        return d3.select(selector).datum(data).call(this);
    }
    return chart;
}
...
(function() {
  var Bubbles, root, rows, texts;

  root = typeof exports !== "undefined" && exports !== null ? exports : this;

  $(function() {
    var display, key, plot, text, word;
    plot = Bubbles();
    ...
    word.keydown(function(e) {
      if (e.keyCode === 13) {
        plot = Bubbles();
        return d3.csv("data/" + text.file, function(data) {
          rows = data;
          return display(rows);
        });
      }
